...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        if use_imed:
            if "mask_info" not in sample or "source_mask" not in sample["mask_info"]:
                raise ValueError("IMED requires mask_info with source_mask in the sample")
            source_mask = sample["mask_info"]["source_mask"]
        else:
            source_mask = None
        ...
        with torch.autograd.profiler.record_function("EnsembleModel: forward_encoder"):
            ...
            if use_imed:
                ...
                sentence_srcs[~source_mask] = self.pad
                sentence_net_input = {
                    **net_input,
                    'src_tokens': sentence_srcs,
                    'src_lengths': source_mask.sum(dim=1)
                }
                sentece_encoder_outs = self.model.forward(sentence_net_input)
            else:
